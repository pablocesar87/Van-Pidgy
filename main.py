...
def main():
    ...
    pygame.mixer.init()
    pygame.mixer.music.load(path.join(SOUND_DIR, "start.ogg"))
    pygame.mixer.music.play()
    showStartScreen()
    while True:
        pygame.mixer.music.stop()
        runGame(map_list=MAP_LIST)
...
#Declaration of images and other sounds-------------------------------------------
    ...
    pidgeon_shot_sound=pygame.mixer.Sound(path.join(SOUND_DIR, "pidgeon_shot.ogg"))
    vampire_dies=pygame.mixer.Sound(path.join(SOUND_DIR, "vampire_dies.ogg"))
    electrocute=pygame.mixer.Sound(path.join(SOUND_DIR, "electrocute.ogg"))
    wind_sound=pygame.mixer.Sound(path.join(SOUND_DIR, "wind_sound.ogg"))
    no_shit_fart=pygame.mixer.Sound(path.join(SOUND_DIR, "no_shit_fart.ogg"))
    pidgeon_bump= pygame.mixer.Sound(path.join(SOUND_DIR, "pidgeon_bump.ogg"))
    pause_sound=pygame.mixer.Sound(path.join(SOUND_DIR, "pause_sound.ogg"))
    ufo_sound=pygame.mixer.Sound(path.join(SOUND_DIR, "ufo_sound.ogg"))
    metalic_bump=pygame.mixer.Sound(path.join(SOUND_DIR, "metalic_bump.ogg"))
    death_shot=pygame.mixer.Sound(path.join(SOUND_DIR, "death_shot.ogg"))
    shot=pygame.mixer.Sound(path.join(SOUND_DIR, "shot.ogg"))
    drone_sound=pygame.mixer.Sound(path.join(SOUND_DIR, "drone.ogg"))
    energy_ball=pygame.mixer.Sound(path.join(SOUND_DIR, "energy_ball.ogg"))
    #------------------------------------------------------------------------
...
#Generation of the vampires----------------------------------------
    ...
    if round_number < 41:
        for i in range(number_of_vampires):
            if round_number>20 and i%2==0:
                vampire_gun=VampireGun(random.choice([1,-1]))
    ...
    #Music of the game
    pygame.mixer.music.load(path.join(SOUND_DIR, "main_game.ogg"))
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == K_LEFT:
                    ...
                    if 0< go_to < 11:
                        runGame(map_list, go_to*5, go_to*0.3, go_to*0.5, total_score, go_to, go_to*2)
                    elif 11<= go_to < 25:
                        runGame(map_list, go_to*4, go_to*0.15, go_to*0.3, total_score, go_to, go_to*2)
                    elif 25<= go_to:
                        runGame(map_list, go_to*3, go_to*0.1, go_to*0.2, total_score, go_to, go_to*2)
                    else:
                        continue
        ...
        for shoting_vampire in vampire_list:
            if round_number < 41:
                if shoting_vampire.get_points()==150:
                    if shoting_vampire.get_shooting_number()==random.randint(1,2000):
                        shot.play()
                        bullet=Bullet()
                ...
                if vampire_boss.get_shooting_number() == random.randint(1, 300):
                    energy_ball.play()
                    bullet = BulletBoss()
                ...
                while True:
                    if checkForKeyPress():
                        ...
                        while True:
                            if checkForKeyPress():
                                pygame.event.get()
                                runGame(map_list, number_of_vampires, vampire_velocity, velocity, total_score, round_number,remaining_shits)
            else:
                while True:
                    if checkForKeyPress():
                        pygame.event.get()
...
def showWinScreen(score, total_score):
    pygame.mixer.music.stop()
    pygame.mixer.music.load(path.join(SOUND_DIR, "you_win.ogg"))
    pygame.mixer.music.play()
...
def showGameOverScreen(score, total_score,round_number):
    pygame.mixer.music.stop()
    pygame.mixer.music.load(path.join(SOUND_DIR, "game_over.ogg"))
    pygame.mixer.music.play()
